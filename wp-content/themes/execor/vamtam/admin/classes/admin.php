...
class VamtamAdmin {
	...
	public static function update_warning() {
		if ( did_action( 'load-update-core.php' ) ) {
			...
			esc_html_e( 'Hey, just a polite reminder that if you update WordPress you will also need to update your theme and plugins.', 'execor' );
			echo '</div></div>';
		}
	...
	public static function icons_selector() {
		...
		<div class="vamtam-config-icons-selector hidden">
			<input type="search" placeholder="<?php esc_attr_e( 'Filter icons', 'execor' ) ?>" class="icons-filter"/>
	...
	public static function theme_upgrade_no_pkg( $reply, $package, $upgrader ) {
		if ( isset( $upgrader->skin->theme_info ) && false !== $upgrader->skin->theme_info ) {
			...
			if ( $theme_slug === VAMTAM_THEME_SLUG && empty( $package ) ) {
				return new WP_Error( 'no_package', __( 'Only Envato Market clients with a valid purchase code are entitled to automatic updates. Envato Elements clients must use FTP.', 'execor' ) );
			}
		}
