...
class VamtamElementorBridge {
	...
	public static function add_sticky_header_bg_color_to_elementor_global_settings() {
		...
		$kit->add_repeater_row( 'system_colors', [
			...
			'title' => __( 'Sticky Header Bg Color', 'execor' ),
	...
	*/
		...
		if ( $use_el_global_defaults ) {
			...
			for ( $i = 1; $i <= 8; $i++ ) {
				...
				$kit->add_repeater_row( 'system_colors', [
					...
					'title' => sprintf( __( 'Accent %s', 'execor' ), $i ),
	...
	protected static function add_theme_fonts_to_elementor_global_fonts() {
		...
		if ( $use_el_global_defaults ) {
			...
			// Adding from vamtam_get_option().
			$our_fonts = [
				'primary-font' => __( 'Primary Font', 'execor' ),
				'h1'           => __( 'H1', 'execor' ),
				'h2'           => __( 'H2', 'execor' ),
				'h3'           => __( 'H3', 'execor' ),
				'h4'           => __( 'H4', 'execor' ),
			...
			foreach ( $our_fonts as $font_prefix => $font_title ) {
				...
				$kit->add_repeater_row( 'system_colors', [
					...
					'title' => sprintf( __( '%s', 'execor' ), $font_title ),
	...
	public static function register_locations( $elementor_theme_manager ) {
		...
		$elementor_theme_manager->register_location(
			'page-title-location',
			[
				'label' => esc_html__( 'Page Title', 'execor' ),
				'multiple' => true,
	...
	public static function get_widget_mods_list() {
		...
		*/
		...
		$widget_mods_list = [
			...
			'button'           => [ 'label' => __( 'Button', 'execor' ) ],
			'icon'             => [ 'label' => __( 'Icon', 'execor' ) ],
			'container'        => [ 'label' => __( 'Container', 'execor' ) ],
			'loop-carousel'    => [ 'label' => __( 'Loop Carousel', 'execor' ) ],
			'nav-menu'         => [ 'label' => __( 'Nav Menu', 'execor' ) ],
			'form'             => [ 'label' => __( 'Form', 'execor' ) ],
			'toggle'           => [ 'label' => __( 'Toggle', 'execor' ) ],
			'text-editor'	   => [ 'label' => __( 'Text Editor', 'execor' ) ],
			'post-comments'    => [ 'label' => __( 'Post Comments', 'execor' ) ],
		];
	...
	*/
	...
	public static function extract_zip( $file, $to ) {
		...
		if ( ! $unzip_result ) {
			$unzip_result = new \WP_Error( 'error', esc_html__( 'Could not unzip or empty archive.', 'execor' ) );
		}
